...
var policyConfig = { 
        "policies" : [
            ...
            },
            {
                ...
                "policyExec" : "containsOnlyCharacters",
                "clientValidation": true,
                "validateOnlyIfPresent": true,
                "policyRequirements": ["CONTAINS_ONLY_CHARACTERS"]
            },
            {
...
function containsOnlyCharacters(fullObject, value, params, property) {
    var allowedCharsPatternString = "^(" + params.allowedCharsPattern + ")+$";
    var allowedCharsPattern = new RegExp(allowedCharsPatternString);
	
    if (typeof(value) !== "string" || !value.length || !allowedCharsPattern.test(value)) {
        return [ {"policyRequirement": "CONTAINS_ONLY_CHARACTERS", "params" : {"allowedCharsPattern": params.allowedCharsPattern} }];
    } else {
        return [];
    }
}
...
function addPolicy(policy) {
    ...
    logger.info ("Policies: {}", policyConfig.policies);
}

function getPolicy(policyId) {
    logger.info ("Passed in policyID: {}", policyId);
    for (var i = 0; i < policyConfig.policies.length; i++) {
        if (policyConfig.policies[i].policyId == policyId) {
           logger.info("current policies policyId: {}", policyConfig.policies[i].policyId);
...
function getAllPolicyRequirements(policies) {
    var reqs = [],i;
    logger.info("getAllPolicyRequirements - passed in policies: {}", policies);
    for (i in policies) {
...
function getPolicyUsingPolicyName (resource) {
    var resourceProperties = resource.properties;
    var policyArray = new Array();
    for (var i = 0; i < resourceProperties.length; i++) {
        var resourcePolicies = resourceProperties[i];
        for (var j = 0; j < resourcePolicies.policies.length; j++) {
            if (resourcePolicies.name == request.params.policyName) {
                policyArray.push(resourcePolicies.policies[j]);
            }
        }
        resourcePolicies.policyId = policyArray;
    }
    return policyArray;
}
...
function processRequest() {
    ...
    if (method == "read") {
        ...
        if (typeof(request.params.policyName) !== "undefined") {
            returnObject = getPolicyUsingPolicyName(resource);
        }
